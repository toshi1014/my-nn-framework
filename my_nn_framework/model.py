...
class Model(LossFunc):
    def __init__(self, *layer_list):
        ...
        if len(self.layer_list) != 1:
            for i in range(1, len(self.layer_list)):
                self.layer_list[i].init_weights(
                    self.layer_list[i-1].out_features)

    def __repr__(self):
        return f"Model with {len(self.layer_list)} layers\n{self.layer_list}"

    def validate(self):
        if self.layer_list[0].in_features is None:
        ...
        for idx, layer in enumerate(self.layer_list):
            try:
                ...
                raise err(f"No out_features in layer[{idx}]")
    ...
    def compile(self, optimizer, loss):
        self.optimizer = optimizer(self.layer_list, loss_func=loss)
